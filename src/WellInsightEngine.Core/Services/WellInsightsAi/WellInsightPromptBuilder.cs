    ...
    public static string Build(string? assetName, string wellName, DateTimeOffset fromUtc, DateTimeOffset toUtc, GroupingInterval interval, WellInsightPayload payload, IReadOnlyList<WellAction> actions)
    {
        ...
        sb.AppendLine("""
            ...
            - highlights: 4–7 пунктів, тільки факт або спостереження.
              ...
              - Формат KPI-пункту:
                "KPI: <метрика> <значення> (<тип агрегації укр., <відхилення%>); <метрика> <значення> (<тип агрегації укр., <відхилення%>); ..."
              - Назва KPI має бути ТІЛЬКИ українською та у формі: "<прикметник агрегації> <метрика>"
              - Відхилення (%):
                - зі знаком + або −
                - без слова "trend"
              - Якщо відхилення неможливо обчислити — пиши "(середнє, n/a)" або відповідний тип агрегації.
              - Якщо KPI взагалі немає або немає даних — KPI-пункт: "KPI: n/a".
